...
int sml_recognition_run_batch(signed short *data_batch, int batch_sz, uint8_t num_sensors, uint32_t sensor_id)
{
	...
	switch(sensor_id)
	{
		case SENSOR_ENG_VALUE_IMU_agm_base + SENSOR_ENG_VALUE_IMU_agm_a_bit + SENSOR_ENG_VALUE_IMU_agm_g_bit:
					ret = kb_run_model((SENSOR_DATA_T *)data, num_sensors, KB_MODEL_TFu_With_SensiML_Features_INDEX);
		if (ret >= 0){
			sml_output_results(KB_MODEL_TFu_With_SensiML_Features_INDEX, ret);
			kb_reset_model(0);
		};
			break;
		case SENSOR_AUDIO:
